		...
		try 
		{
			...
			for (String type : new String[]{"PCFG", "Factored"})
			{
				...
				for (int i = 1; i <= corpus.getArticles().size(); i++)
				{
					...
					// DEBUG: used to only process first article for debugging
					// break;
				}
			}
			...
			System.out.println("\nTIME ELAPSED TO PARSE CORPUS:");
			System.out.println("Milliseconds:\t" + elapsedTimeMillis);
			System.out.println("Seconds:\t" + elapsedTimeMillis/1000F);
			System.out.println("Minutes:\t" + elapsedTimeMillis/(60*1000F));
			System.out.println("Hours:\t\t" + elapsedTimeMillis/(60*60*1000F));
			System.out.println("Days:\t\t" + elapsedTimeMillis/(24*60*60*1000F));
		}
		catch (Exception e) 
		{
			e.printStackTrace();
